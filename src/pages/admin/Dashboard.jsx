    ...
    <div className="min-h-screen bg-[color:var(--color-cream)] flex flex-col">
      <header className="border-b border-[color:var(--color-cream-deep)] bg-white">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 py-4 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3">
            ...
            <span className="font-bold">لوحة المشرف</span>
          </Link>
          <div className="flex items-center gap-3">
            <span className="text-sm text-[color:var(--color-ink-soft)]" dir="ltr">{user?.email}</span>
            <button
              onClick={signOut}
      ...
      <main className="flex-1 flex items-center justify-center p-8">
        <div className="max-w-lg text-center">
          ...
          <h1 className="font-display text-3xl font-bold mb-3">أهلاً يا معلم</h1>
          <p className="text-[color:var(--color-ink-soft)] leading-relaxed">
            هذه اللوحة قيد التطوير. ستحوي قريبًا: صندوق الوارد لتأملات الطلاب، قائمة الطلاب مع تقدّمهم، ومحرّر محتوى الأسماء.
          </p>
        </div>
      </main>
    </div>
