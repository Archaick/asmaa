...
export default function CelebrationOverlay({ open, bouquetTitle, subtitle, onClose }) {
  const { t } = useLang()

  useEffect(() => {
    if (!open) return
    const to = setTimeout(() => onClose?.(), 3200)
    return () => clearTimeout(to)
  }, [open, onClose])

  if (!open) return null

  const title = `${t('celebration.bouquet_complete_prefix')} ${bouquetTitle}`
  const sub = subtitle || t('celebration.subtitle')
        ...
        {Array.from({ length: SPARKLES }).map((_, i) => {
          ...
          return (
            <div
              ...
              style={{ '--r': `${rot}deg`, animationDelay: `${(i % 5) * 60}ms` }}
            >
